...
function buildBook(name, geners) {
  //geners is an []
  var book = {};
  book.name = name;
  book.geners = geners;
...
var library = [
  buildBook("The Big Book of Classic Fantasy", [
    "Fantasy",
    "Classics",
    "FairyTail",
  ]),
  ...
  buildBook("Blood River: A Journey to Africa's Broken Heart", [
    "Historical",
    "Adventure",
  ]),
...
function compairTwoArrays(arrayOfArrays, array) {
  //a geniral function to use later to get the array of indexes.
  var acc = [];
  each(arrayOfArrays, function (element, index) {
    for (var i = 0; i < array.length; i++) {
      if (element.includes(array[i])) acc.push(index);
    }
  }); /// try using each instead of for.
  return acc;
}

var search = function (library, input) {
  //i need to compair the input[] with the gener[] which is inside every obj of the library
